...
export default class CustomerCreate extends Mixins {
  ...
  protected getOptions(attribute: any) {
    ...
    switch (attributeInputTypeId) {
      ...
      default:
        const selectedAttribute = this.customerAttributes.find((t) => t.id === attributeId)
  ...
  protected async onSubmit(bvModalEvt: any) {
    ...
    const onSuccess = () => {
      ...
      this.$bvModal.hide('customerCreateModal')
      this.toastSuccess('Thêm khách hàng thành công.')
    }
    const onFail = (res: any) => {
      const resData = res.data
      this.serverErrors = resData.errors
      this.toastError('Thêm khách hàng thất bại.')
    }
    this.customerApi.create({ attributes: data, created_by: this.currentUser.id }).then((res) => {
  ...
  private createForm() {
    const hiddenAtts = ['ma_kh']
    const attributes = this.customerAttributes.filter((t) => t.is_default)
    this.attributes = []
