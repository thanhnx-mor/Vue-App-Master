...
<template>
  <div class="page-list">
    <div class="body-upper">
      ...
      <div class="actions">
        <Create @onCreated="onRefresh" v-if="customerAttributes && customerAttributes.length" />
    ...
    <template>
      ...
      <div class="body-content">
          ...
          <div class="row">
            <div class="col-md-6 mb-4">
              <template v-if="items.length > 0">
                ...
                >
                  <template
                    v-if="hasPermission([PERMISSION.CUSTOMER['*'], PERMISSION.CUSTOMER.EXPORT])"
                  >
                    <b-overlay
                      rounded
                      opacity="0.6"
                    ...
                    >
                      <b-dropdown-item @click.prevent="customersExport()" class="action-items">
            ...
            <div class="col-md-6 mb-4">
              <b-input-group class="search float-right" id="search_box" v-b-popover.hover.top>
                <b-popover
                  ...
                  placement="top"
                >
                  <template v-slot:title>Bạn có thể tìm kiếm khách hàng theo</template>
                  <div class="suggest-search-item">- Tên KH</div>
          ...
          <template>
            ...
            >
              ...
              <template v-slot:cell(actions)="data">
                <template
                  v-if="hasPermission([PERMISSION.CUSTOMER['*'], PERMISSION.CUSTOMER.DESTROY])"
                >
              ...
              <template v-slot:cell(ten_kh)="data">
                <template
                  v-if="hasPermission([PERMISSION.CUSTOMER['*'], PERMISSION.CUSTOMER.SHOW])"
                >
                  <router-link
                    :to="{ name: PERMISSION.CUSTOMER.SHOW, params: { id: data.item.id } }"
        ...
        <template v-if="!items.length && !isLoading">
          ...
          <div v-else class="text-center text-confirm">Chưa có khách hàng nào.</div>
        </template>
      </div>
    </template>
    <b-modal
    ...
    >
      <div class="text-confirm text-center">
        ...
        <!-- Hiện tại hệ thống có thể trích xuất nhỏ hơn hoặc bằng {{ maxRowExport }} KH. -->
      </div>
    </b-modal>
    <b-modal
      id="exportProgress"
