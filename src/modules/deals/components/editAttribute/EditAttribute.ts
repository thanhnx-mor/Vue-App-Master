...
@Component({
  ...
  computed: {
    ...mapGetters(['dealAttributes', 'activeUsers']),
  },
})
export default class EditDealAttribute extends Mixins {
  ...
  protected get options() {
    switch (this.attribute?.attribute_input_type_id) {
      ...
      default:
        const selectedAttribute = this.dealAttributes.find((t) => t.id === this.attribute?.id)
