...
import CreatePayment from '@/modules/deals/components/payment/create/Create'
import ShowPayment from '@/modules/deals/components/payment/show/Show'
...
export default class DealPaymentList extends DealListCommon {
  ...
  protected sortOrder: { [key: string]: number } = {
    ma_dh: 1, ten_dh: 2, ngay_dat_hang: 3, trang_thai_thanh_toan: 3, ma_kh_dh: 6, ten_kh_dh: 7, sdt_kh_dh: 8,
  ...
  public destroyed() {
    ...
    EventBus.$off('showModalDealPayment')
    EventBus.$off('showModalDealDetailsPayment')
  }
  ...
  protected showModalDealPayment(data: any) {
    EventBus.$emit('showModalDealPayment', data)
  }

  protected showModalDealDetailsPayment(data: any) {
    EventBus.$emit('showModalDealDetailsPayment', data)
  }
  ...
  protected setQueryUrl(params: { page: number, search?: string, stage_id?: number, lead_type_id?: number, action_id?: number } =
    ...
    this.$nextTick(() => {
      this.$router.replace({
        name: this.PERMISSION.DEAL.PAYMENT.INDEX,
        query: {
