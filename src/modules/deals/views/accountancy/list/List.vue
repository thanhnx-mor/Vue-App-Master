  ...
  <div class="page-list">
    ...
    <div class="statistical mt-3">
      ...
      >
        ...
        <ul class="custom-list list-inline" v-if="stats.deal_stages">
          <template v-for="(item, index) in stats.deal_stages">
            <li
        ...
        <ul class="custom-list list-inline" v-if="stats.deal_lead_types">
          <template v-for="(item, index) in stats.deal_lead_types">
            <li
              class="custom-list-item"
              :key="index"
        ...
        <h5>Hành động</h5>
        <ul class="custom-list list-inline" v-if="stats.deal_actions">
          <template v-for="(item, index) in stats.deal_actions">
            <li
              class="custom-list-item"
              :key="index"
        ...
        <b-overlay :show="isLoading" rounded opacity="0.6" spinner-small spinner-variant="primary">
          ...
          <template>
            <b-table
              ...
              :items="items"
              :fields="fields"
              class="deal-table deal-table-payment"
            ...
            >
              ...
              <template v-slot:cell(actions)="data">
                <button
                  @click.prevent="showModalDealPayment({deal_id: data.item.id})"
                  variant="primary"
                  class="nt_btn nt_btn-warning"
                  size="sm"
                >Thêm thanh toán</button>
              </template>
              <template v-slot:cell(ma_dh)="data">
                <a
                  href="#"
                  @click.prevent="showModalDealDetailsPayment({deal_id: data.item.id})"
                >{{ data.value }}</a>
              </template>
    ...
    <CreatePayment @onCreated="onRefresh" />
    <ShowPayment />
  </div>
</template>