...
import { GET_DEAL_ATTRIBUTES, GET_DEAL_ACTION_RESULTS, GET_DEAL_STAGES, GET_DEAL_LEAD_TYPES, GET_DEAL_ACTIONS, GET_DEAL_WORKFLOWS } from '../../../type'
...
import CreateAction from '@/modules/deals/components/me/createAction/CreateAction'
import Show from '@/modules/deals/components/me/show/Show'
...
export default class DealMeList extends DealListCommon {
  ...
  protected isLoadingStatsByMe: boolean = false
  protected statsByMe: { [key: string]: string | number } = {}
  ...
  public destroyed() {
    ...
    EventBus.$off('showModalDealAction')
    EventBus.$off('showModalDealDetailsAction')
  }
  ...
  protected async dealsExport() {
    ...
    for (let i = 1; i <= numberLoop; i++) {
      const res: any = await this.dealApi.exportByMe({
        page: 1,
  ...
  protected showModalDealAction(data: any) {
    EventBus.$emit('showModalDealAction', data)
  }

  protected showModalDealDetailsAction(data: any) {
    EventBus.$emit('showModalDealDetailsAction', data)
  }

  protected onCreatedAction(data: any) {
    const query = { page: 1, ...this.filterCustom }
    this.setQueryUrl(query)
    this.$nextTick(() => {
      const deal: any = this.items.find((t: any) => t.id === data.id)
      deal.deal_stages = this.getDealLeadTypeName(data.latest_action_history.stage_id)
      deal.deal_lead_types = this.getDealStageName(data.latest_action_history.lead_type_id)
      this.getStatsByMe()
      this.$forceUpdate()
    })
  }
  ...
  protected setQueryUrl(params: { page: number, search?: string, stage_id?: number, lead_type_id?: number, action_id?: number } =
    ...
    this.$nextTick(() => {
      this.$router.replace({
        name: this.PERMISSION.DEAL.ME.INDEX,
        query: {
  ...
  private getStatsByMe() {
    this.isLoadingStatsByMe = true
    this.dealApi.statsByMe().then((res: any) => {
      const data = res.data
      const firtRow = {
        name: 'Tất cả',
      ...
      this.isLoadingStatsByMe = false
    })
  }

  private async getData() {
    this.$store.dispatch(GET_DEAL_WORKFLOWS)
  ...
  private fetch() {
    ...
    this.dealApi.filterByMe({ page: this.currentPage, search: this.searchKey, filter }).then((res: any) => {
