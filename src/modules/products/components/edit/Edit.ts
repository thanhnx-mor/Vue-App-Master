...
export default class ProductEdit extends Mixins {
  ...
  protected uneditableField: string[] = ['ma_sp']
  ...
  protected get options() {
    switch (this.attribute?.attribute_input_type_id) {
      ...
      default:
        const selectedAttribute = this.productAttributes.find((t) => t.id === this.attribute?.id)
  ...
  protected get isEditable() {
    if (this.attribute) {
      return !['ma_sp'].includes(this.attribute.slug)
    }
  }
