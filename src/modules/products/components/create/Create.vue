...
<template>
  <div>
    <template v-if="hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.STORE])">
      <b-button v-b-modal.productCreateModal class="nt_btn nt_btn-warning nt-button-top">
          ...
          <b-form @submit.stop.prevent="passes(onSubmit)" autocomplete="true">
            <template v-for="(attribute, index) in attributes">
              ...
              >
                ...
                >
                  <!-- <template v-if="attribute.slug === 'ten_sp'">
                  <b-form-input
                    :id="attribute.slug"
                    :name="attribute.slug"
                    ...
                    @keyup="setDefaultValueSKU(); resetAttributeError(attribute.slug, serverErrors)"
                    @blur="setDefaultValueSKU();"
                  ></b-form-input>
                  </template>-->
