...
<template>
  <div class="page-list">
    <div class="body-upper">
      ...
      <div class="actions">
        <Create @onCreated="onRefresh" v-if="productAttributes && productAttributes.length" />
    ...
    <template>
      ...
      <div class="body-content">
          ...
          <div class="row">
            <div class="col-md-6 mb-4">
              <template v-if="items.length > 0">
                ...
                >
                  <template
                    v-if="hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.EXPORT])"
                  >
                    <b-overlay
                      rounded
                      opacity="0.6"
                    ...
                    >
                      <b-dropdown-item @click.prevent="productsExport()" class="action-items">
            ...
            <div class="col-md-6 mb-4">
              <b-input-group class="search float-right" id="search_box" v-b-popover.hover.top>
                <b-popover
                  ...
                  placement="top"
                >
                  <template v-slot:title>Bạn có thể tìm kiếm sản phẩm theo</template>
                  <div class="suggest-search-item">- Tên SP</div>
                  <div class="suggest-search-item">- Mã SKU</div>
                </b-popover>
          ...
          <template>
            ...
            >
              ...
              <template v-slot:cell(actions)="data">
                <template
                  v-if="hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.DESTROY])"
                >
              ...
              <template v-slot:cell(ten_sp)="data">
                <template v-if="hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.SHOW])">
                  <router-link
                    :to="{ name: PERMISSION.PRODUCT.SHOW, params: { id: data.item.id } }"
        ...
        <template v-if="!items.length && !isLoading">
          ...
          <div v-else class="text-center text-confirm">Chưa có sản phẩm nào.</div>
        </template>
      </div>
    </template>
