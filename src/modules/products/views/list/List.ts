...
export default class ProductList extends Mixins {
  ...
  protected onDelete(product: any) {
    const { deleteModal } = this.$refs as any
    deleteModal.name = product.ten_sp
  ...
  protected async productsExport() {
    ...
    for (let i = 1; i <= numberLoop; i++) {
      const res: any = await this.productApi.export({
        page: 1,
  ...
  private setQueryUrl(params: { page: number, search?: string } = { page: 1, search: '' }) {
    if (this.currentPage !== +this.$route.query.page ||
      ...
      this.$router.replace({
        name: this.PERMISSION.PRODUCT.INDEX,
        query: {
  ...
  private fetch() {
    ...
    this.productApi.filter({ page: this.currentPage, search: this.searchKey, filter: this.filter }).then((res: any) => {
