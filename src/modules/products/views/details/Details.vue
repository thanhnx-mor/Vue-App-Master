...
<template>
  <div>
    <div class="body-upper">
      ...
      <div class="actions">
        <template v-if="hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.DESTROY])">
          <b-button
              ...
              <template v-for="(attribute, index) in product.attributes">
                <div class="col-sm-6 pt-2 pb-2" :key="index" v-if="attribute.is_enabled">
                  <div
                    ...
                    style="margin: 0px;"
                    :class="[
                    attribute.slug === 'ma_sp' ? 'nt-attribute-item-disabled' : '',
                    !hasPermission([PERMISSION.PRODUCT['*'], PERMISSION.PRODUCT.UPDATE])? 'nt-attribute-item-disabled' : ''
                    ]"
                  >
                    <label for="product_group" class="form-control-label font-weight-bold">
                      <span :class="{ 'required': attribute.is_required }">
                        {{ attribute.name }}
                        <i
                          v-if="attribute.slug === 'ma_sp'"
