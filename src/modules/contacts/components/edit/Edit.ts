...
export default class ContactEdit extends Mixins {
  ...
  protected uneditableField: string[] = ['ma_lh']
  ...
  protected get options() {
    switch (this.attribute?.attribute_input_type_id) {
      ...
      default:
        const selectedAttribute = this.contactAttributes.find((t) => t.id === this.attribute?.id)
  ...
  protected get isEditable() {
    if (this.attribute) {
      return !['ma_lh'].includes(this.attribute.slug)
    }
  }
