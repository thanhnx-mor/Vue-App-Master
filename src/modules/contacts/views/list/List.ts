...
export default class ContactList extends Mixins {
  ...
  protected onDelete(contact: any) {
    ...
    deleteModal.name = contact.ten_lh
  ...
  protected async contactsExport() {
    ...
    for (let i = 1; i <= numberLoop; i++) {
      const res: any = await this.contactApi.export({
        page: 1,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'DANH_SACH_LH')

    XLSX.writeFile(wb, 'DANH_SACH_LH.xlsx')
  ...
  private setQueryUrl(params: { page: number, search?: string } = { page: 1, search: '' }) {
    if (this.currentPage !== +this.$route.query.page ||
      ...
      this.$router.replace({
        name: this.PERMISSION.CONTACT.INDEX,
        query: {
  ...
  private buildFields() {
    ...
    this.contactAttributes.forEach((attribute) => {
  ...
  private fetch() {
    ...
    this.contactApi.filter({ page: this.currentPage, search: this.searchKey, filter: this.filter }).then((res: any) => {
