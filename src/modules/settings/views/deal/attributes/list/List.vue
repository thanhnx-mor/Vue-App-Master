...
<template>
  <div>
    <div class="body-upper">
      ...
      <div class="actions">
        <template
          v-if="hasPermission([PERMISSION.DEAL['*'], 
        PERMISSION.DEAL.ATTRIBUTE['*'], PERMISSION.DEAL.ATTRIBUTE.STORE])"
        >
          <b-button
        ...
        >
          <template v-slot:cell(actions)="data">
            <template v-if="!data.item.is_default">
              <template
                v-if="hasPermission([PERMISSION.DEAL['*'], 
                PERMISSION.DEAL.ATTRIBUTE['*'], PERMISSION.DEAL.ATTRIBUTE.UPDATE])"
              >
              ...
              <template
                v-if="hasPermission([PERMISSION.DEAL['*'], 
                PERMISSION.DEAL.ATTRIBUTE['*'], PERMISSION.DEAL.ATTRIBUTE.DESTROY])"
              >
    ...
    <Create @onCreated="onRefresh" :createAction="GET_DEAL_ATTRIBUTE_CREATE" :prefix="prefix" />
    <Edit
      @onUpdated="onRefresh"
      :detailsAction="GET_DEAL_ATTRIBUTE_SHOW"
      :updateAction="GET_DEAL_ATTRIBUTE_UPDATE"
      :prefix="prefix"
    />
    <Delete
      @onDeleted="onRefresh"
      :deleteAction="GET_DEAL_ATTRIBUTE_DELETE"
      :checkBeforeDeleteAction="GET_DEAL_CHECK_BEFORE_ATTRIBUTE_DELETE"
      :prefix="prefix"
    />
  </div>
</template>