...
@Component({
  ...
  computed: {
    ...mapGetters(['dealAttributes', 'attributeInputTypes']),
  },
})

export default class ListDealAttribute extends Mixins {

  protected breadcrumbItems: IBreadcrumb[] = [{
    text: 'Trang chủ',
    to: '/',
  }, {
    text: 'Cài đặt trường thông tin ĐH',
    active: true,
  }]

  protected dealAttributes!: IAttribute[]
  protected attributeInputTypes!: IAttributeInputType[]
  protected GET_DEAL_ATTRIBUTE_CREATE: string = GET_DEAL_ATTRIBUTE_CREATE
  protected GET_DEAL_ATTRIBUTE_SHOW: string = GET_DEAL_ATTRIBUTE_SHOW
  protected GET_DEAL_ATTRIBUTE_UPDATE: string = GET_DEAL_ATTRIBUTE_UPDATE
  protected GET_DEAL_ATTRIBUTE_DELETE: string = GET_DEAL_ATTRIBUTE_DELETE
  protected GET_DEAL_CHECK_BEFORE_ATTRIBUTE_DELETE: string = GET_DEAL_CHECK_BEFORE_ATTRIBUTE_DELETE
  protected prefix: string = '_dh'
  ...
  get attributes() {
    if (!this.dealAttributes.length) {
      return []
    }

    return this.dealAttributes.map((t: IAttribute) => {
      return {
        ...t,
  ...
  private fetchDealAttributes() {
    this.$store.dispatch(GET_DEAL_ATTRIBUTES)
  }
