...
<template>
  <div>
    <div class="body-upper">
      ...
      <div class="actions">
        <template
          v-if="hasPermission([PERMISSION.PRODUCT['*'], 
        PERMISSION.PRODUCT.ATTRIBUTE['*'], PERMISSION.PRODUCT.ATTRIBUTE.STORE])"
        >
          <b-button
        ...
        >
          <template v-slot:cell(actions)="data">
            <template v-if="!data.item.is_default">
              <template
                v-if="hasPermission([PERMISSION.PRODUCT['*'], 
                PERMISSION.PRODUCT.ATTRIBUTE['*'], PERMISSION.PRODUCT.ATTRIBUTE.UPDATE])"
              >
              ...
              <template
                v-if="hasPermission([PERMISSION.PRODUCT['*'], 
                PERMISSION.PRODUCT.ATTRIBUTE['*'], PERMISSION.PRODUCT.ATTRIBUTE.DESTROY])"
              >
    ...
    <Create @onCreated="onRefresh" :createAction="GET_PRODUCT_ATTRIBUTE_CREATE" :prefix="prefix" />
    <Edit
      @onUpdated="onRefresh"
      :detailsAction="GET_PRODUCT_ATTRIBUTE_SHOW"
      :updateAction="GET_PRODUCT_ATTRIBUTE_UPDATE"
      :prefix="prefix"
    />
    <Delete
      @onDeleted="onRefresh"
      :deleteAction="GET_PRODUCT_ATTRIBUTE_DELETE"
      :checkBeforeDeleteAction="GET_PRODUCT_CHECK_BEFORE_ATTRIBUTE_DELETE"
      :prefix="prefix"
    />
  </div>
</template>