...
<template>
  <div>
    <div class="body-upper">
      ...
      <div class="actions">
        <template
          v-if="hasPermission([PERMISSION.CONTACT['*'], 
        PERMISSION.CONTACT.ATTRIBUTE['*'], PERMISSION.CONTACT.ATTRIBUTE.STORE])"
        >
          <b-button
            v-b-modal.attributeCreateModal
            variant="primary"
            class="float-right nt_btn nt_btn-warning nt-button-top"
          >
        ...
        <template v-else>
          <b-button
            variant="primary"
            class="float-right nt_btn nt_btn-warning nt-button-top permission-btn-disabled"
            v-b-tooltip.hover
        ...
        >
          <template v-slot:cell(actions)="data">
            <template v-if="!data.item.is_default">
              <template
                v-if="hasPermission([PERMISSION.CONTACT['*'], 
                PERMISSION.CONTACT.ATTRIBUTE['*'], PERMISSION.CONTACT.ATTRIBUTE.UPDATE])"
              >
              ...
              <template
                v-if="hasPermission([PERMISSION.CONTACT['*'], 
                PERMISSION.CONTACT.ATTRIBUTE['*'], PERMISSION.CONTACT.ATTRIBUTE.DESTROY])"
              >
    ...
    <Create @onCreated="onRefresh" :createAction="GET_CONTACT_ATTRIBUTE_CREATE" :prefix="prefix" />
    <Edit
      @onUpdated="onRefresh"
      :detailsAction="GET_CONTACT_ATTRIBUTE_SHOW"
      :updateAction="GET_CONTACT_ATTRIBUTE_UPDATE"
      :prefix="prefix"
    />
    <Delete
      @onDeleted="onRefresh"
      :deleteAction="GET_CONTACT_ATTRIBUTE_DELETE"
      :checkBeforeDeleteAction="GET_CONTACT_CHECK_BEFORE_ATTRIBUTE_DELETE"
      :prefix="prefix"
    />
  </div>
</template>